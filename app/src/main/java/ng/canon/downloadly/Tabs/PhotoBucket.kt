package ng.canon.downloadly.Tabs
...
import ng.canon.downloadly.GlideApp
import ng.canon.downloadly.R
import java.io.File
import java.util.*
...
class PhotoBucket : Fragment(),SwipeRefreshLayout.OnRefreshListener {
    ...
    fun FileQuery() {
        ...
        arrayList!!.clear()
        path = Environment.getExternalStorageDirectory().absolutePath + "/downloadly/"
        val dex = File(Environment.getExternalStorageDirectory().absolutePath, "downloadly")
        if (!dex.exists())
            dex.mkdirs()
